...
namespace Ex01.FacebookClient
{
    public partial class Form1 : Form
    {
        ...
        private void loginAndInit()
        {
            ...
            LoginResult result = FacebookService.Login("225454867885514", /// (desig patter's "Design Patterns Course App 2.4" app)
                "public_profile",
        ...
        private void btnGo_Click(object sender, EventArgs e)
        {
            ...
            foreach (Checkin checkin in m_LoggedInUser.Checkins)
            {
                if (checkin.CreatedTime.Value >= from && checkin.CreatedTime.Value <= to)
                {
                    checkins.Add(checkin);
                }
            }

            string[] locations = new string[checkins.Count];

            for (int i = 0; i < locations.Length; i++)
            {
                if (checkins[i].Place.Location != null)
                {
                    locations[i] = checkins[i].Place.Location.Latitude + "," + checkins[i].Place.Location.Longitude;
                }

            }

            pictureBoxMap.Load(getMapURL(locations));
        }

        private string getMapURL(string[] locations)
        {
            string url = "https://maps.googleapis.com/maps/api/staticmap?size=798x460";
            string markers = "&markers=color: red";
            string path = "&path=color:0x0000ff|weight:5";

            foreach (string location in locations)
            {
                string urlEncodedLocation = HttpUtility.UrlEncode(location);
                markers += "|" + urlEncodedLocation;
                path += "|" + urlEncodedLocation;
            }
